...
def main():
    ...
    # converte if frames in bianco e nero in frame
    ...
    count = 0
    while success:
        cv2.imwrite(os.path.join(black_and_white_frames_path, f"frame{count}.jpg"), image)
        success, image = vidcap.read()
        count += 1
    ...
    count -= 1
    # quantità file presenti nella cartella dei frame
    print("Frame number " + str(count))

    # converte i frame in bianco e nero a colori
    numero = 0
    for i in range(count):
        numero += 1
        img_path = os.path.join(black_and_white_frames_path, 'frame{}.jpg'.format(numero))
        print(img_path)
        ...
        out_file = os.path.join(colored_frames_path, f'{numero}.jpg')
